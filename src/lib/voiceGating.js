...
function distBucket(m) {
  if (m >= 9) return "f";
  return String(Math.floor(m / 3));
}

function zoneBucket(o) {
  if (o.distanceMeters >= 8) return "x";
  return o.zone === "center" ? "c" : "s";
}
...
export function computeUrgent(obstacles) {
  ...
  const key = `${String(n.class).toLowerCase()}:${Math.floor(n.distanceMeters / 2)}:${n.zone}`;
  return { key, nearest: n };
}

const URGENT_MIN_GAP_MS = 7500;
const URGENT_SAME_KEY_REPEAT_MS = 22000;
const NON_URGENT_MIN_GAP_MS = 34000;
...
export function decideVoiceUtterance({
  ...
  // Non-urgent: only when stable scene signature meaningfully changed + cooldown
